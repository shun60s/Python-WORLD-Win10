...
#                d4c-Requiem is new waveform generator method from WORLD version 0.2.2 against d4c
#                Input
#                     x  : input signal
...
def d4cRequiem(x: np.ndarray, fs: int, f0_object: dict, threshold: float=0.85, fft_size: int=None) -> dict:
    f0_low_limit = 47
    if fft_size == None:
    ...
    number_of_aperiodicities = int(np.floor(np.min([upper_limit, fs / 2 - frequency_interval]) / frequency_interval))
    assert number_of_aperiodicities > 0
    ...
    band_aperiodicity = np.zeros((number_of_aperiodicities + 2, len(f0)))
    band_aperiodicity[0,:] = -60
    band_aperiodicity[-1,:] = -0.000000000001

    for i in range(len(f0)):

        if d4c_love_train(x, fs, f0[i], temporal_positions[i], threshold) == 0:
        ...
        band_aperiodicity[1:-1,i] = -np.maximum(0, coarse_aperiodicity - (current_f0 - 100) * 2 / 100)

    f0_object['aperiodicity'] = band_aperiodicity
...
def get_windowed_waveform(x: np.ndarray, fs: int, current_f0: float,
                          current_position: float, half_length: float,
                          window_type: int) -> np.ndarray:  # 1: hanning, 2: blackman
