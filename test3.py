...
if __name__ == '__main__':
    ...
    parser.add_argument('--inFILE', '-i', default='na_1_48k.wav', help='input wav file')
    ...
    if 1: # show temporal_positions list dump
        print ('*** temporal_positions list ***', len(dat['temporal_positions']) ) # temporal position is not pitch position itself
        print ( dat['temporal_positions'])
    ...
    if 0:  # visualize for spectrum comparison
        vocoder.draw2(x, dat)
    if 1:  # visualize for spectrum comparison and visualize fundamental frequency
        vocoder.draw3(x, dat)
